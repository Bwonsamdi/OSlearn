...
中断向量3通常用于允许调试器通过临时用特殊的1字节的int3软件中断指令替换相关的程序指令来在程序代码中插入断点。JOS将该异常用作简单的伪系统调用，用户环境可以使用此异常唤起内核监视器。如果我们将JOS内核监视器看作一个简陋的调试器，这种做法实际上就是合适的。用户态中panic()的实现位与lib/panic.c中。
